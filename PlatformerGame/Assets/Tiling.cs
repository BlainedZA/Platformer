...
public class Tiling : MonoBehaviour {
	...
	public int offsetX = 2;		// offset so we dont get any wierd errors
	...
	void Start () {
		...
		spriteWidth = sRenderer.sprite.bounds.size.x;		// calculate the length of the sprite
	}
	
	// Update is called once per frame
	void Update () {

		// checking if it still needs buddies. If not do nothing.
		if (hasALeftBuddy == false || hasARightBuddy == false)
        {
			// calculate the cameras extend (half the width) of that what the camera can see in world coordinates
